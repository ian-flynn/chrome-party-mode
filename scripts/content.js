...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
  if (request.action === "toggle_sephia") {
    ...
    if(request.onOff === 'on'){
      ...
      giant.setAttribute('id', 'giant-mask')
      ...
      document.querySelector('#giant-mask').remove()
    }
    ...
    if(request.onOff === 'on'){
      ...
      anotherGiant.setAttribute('id', 'another-giant-mask')
      anotherGiant.setAttribute('style', 'z-index: 1000; width: 100%; min-height: 100%; background-color: black; position: fixed; top: 0; left: 0; opacity: 0.5;')
      let body = document.querySelector('body')
      body.appendChild(anotherGiant)
    } else {
      document.querySelector('#another-giant-mask').remove()
    }
    ...
    if(request.onOff === 'on'){
      ...
      oneMoreGiant.setAttribute('id', 'one-more-giant-mask')
      ...
      document.querySelector('#one-more-giant-mask').remove()
    }
  }
});
